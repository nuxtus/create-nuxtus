...
if (process.argv.length < 3) {
  console.error(chalk.red('You have to provide a name for your app.'));
  console.error('For example :');
  console.error('    npx create-nuxtus ' + chalk.bold('my-app'));
  process.exit(1);
}
...
async function main(): Promise<void> {
  try {
    ...
    credentials = await inquirer.prompt(
      ...
      (databaseQuestions[dbClient] as any[]).map(
        (question: ({ client, filepath }: any) => any) =>
  ...
  console.info(''); // empty line between Directus questions and status messages
