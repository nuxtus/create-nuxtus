...
export async function askOptions(): Promise<Options> {
  console.info('Directus configuration\n');
  return inquirer.prompt([
    {
      ...
      validate: (input: string): boolean => {
        ...
        if (error)
          throw new Error(
            'The Directus Cloud URL must be a valid URL or empty.',
          );
        return true;
      },
    },
...
export async function cleanUp(projectName: string): Promise<void> {
  return new Promise((resolve, reject) => {
    try {
      execSync(
        `npx rimraf ./.git ./TODO ./node_modules ./.github ./CHANGELOG.md ./package.json ./package-lock.json ./LICENSE .gitignore renovate.json`,
      );
      ...
      // Create .gitignore
      const gitignoreTemplateString = fs.readFileSync(
        path.join(process.cwd(), 'templates', 'gitignore.liquid'),
        'utf8',
      );
      const gitignoreTemplate = liquidEngine.parseAndRenderSync(
        gitignoreTemplateString,
      );
      fs.writeFileSync(
      ...
      const packageTemplateString = fs.readFileSync(
        path.join(process.cwd(), 'templates', 'package.json.liquid'),
        'utf8',
      );
